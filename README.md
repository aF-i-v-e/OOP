...
Убрать static у методов ConsoleOperations, в контроллере создать объект этого класса и все методы у него вызвать.
Тоже сделать для класса CommandHandler.
Логику из контроллера перенести в CommandHandler.
Возвращаемые данные из обработчика сделать отдельным классом. botRequest и answer.